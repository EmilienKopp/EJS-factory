...
export default class EJSHtmlBuilder {
    ...
    /**
     * Collection of icon CSS links to import
     * @static
     * @memberof EJSHtmlBuilder
     */
    static icons = {
      'bootstrap': {
        CDN: 'https://cdn.jsdelivr.net/npm/bootstrap-icons@1.9.1/font/bootstrap-icons.css',
        classes: ['bi'],
      }
    }
    ...
    constructor(data = null) {
      ...
      this.builder.attaches = function(block, container) {
        ...
        let attachesBrick = new HtmlBricks().on(container)
                                .fromBlock(block)
                                .identify('attaches'+ Math.round((100*Math.random())))
                                .withAttributes([
                                  {name: 'href', value: url},
                                ]);
        console.log('attachesBrick.htmlElement', attachesBrick.htmlelement);
        attachesBrick.lay();
        ...
        EJSHtmlBuilder.addIconCSS();
        let extension = filename.split('.').pop();

        let fileIconBrick = new HtmlBricks().on(fileBlockBrick)
                              .fromProperty('icon')
                              .withClasses(['bi-filetype-'+extension]);
        console.log('fileIconBrick.htmlElement', fileIconBrick.htmlelement);
        fileIconBrick.lay();
        ...
        let downloadIconBrick = new HtmlBricks().on(fileBlockBrick)
                                    .fromProperty('icon')
                                    .withClasses(['bi-download']);

        downloadIconBrick.lay();
     ...
     */
      ...
      /**
       * Set the icon CSS library to use for the icons
       * @static
       * @param {string} libraryName Name of the library to use
       * @returns {void}
       * @memberof HtmlBricks
       */
      static setIconLibrary(libraryName) {
        EJSHtmlBuilder.iconCSS = libraryName;
      }

      /**
       * Add a <link> tag to the head of the document only if it is not already there to import CSS for the icons
       * @static
       * @param {string} href URL of the CDN to import the CSS from
       * @returns {HtmlBricks} return brick for chaining
       */
      static addIconCSS() {
        let href = EJSHtmlBuilder.icons[EJSHtmlBuilder.iconCSS].CDN;
