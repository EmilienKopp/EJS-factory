...
export const HtmlBricks = HtmlBricks;
...
export default class EJSHtmlBuilder {
     ...
     * @param {boolean} rawe Raw mode (build from hard coded text) or not, defaults to false
     * @returns 
     */
    build(target, raw = false) {
      ...
      if (this.rawMode) {
        ...
        console.time('Execution time - txt');
        ...
        console.timeEnd('Execution time - txt');
        target.innerHTML = this.html;
        return this.html;

      } else {
        console.time('Execution time - bricks');
        // Using HTML Bricks
        ...
        console.timeEnd('Execution time - bricks');
