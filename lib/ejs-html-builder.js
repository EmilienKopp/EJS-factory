...
export default class EJSHtmlBuilder {
    ...
    constructor(data = null) {
      ...
      this.builder.code = function(block, container) {
        ...
        //TODO : Sanitize code in block.data.code to prevent XSS
        let cleanCode = block.data.code.replace(/</g, '&lt;').replace(/>/g, '&gt;');
        Logger.SmartLog(2, 'Sanitized code: ',cleanCode);

        let highlightedHTML = hljs.highlightAuto(block.data.code).value;

        let codeBrick = new HtmlBricks().on(preBrick)
                            .fromBlock(block)
                            .withText(highlightedHTML);
        codeBrick.lay();
      }
